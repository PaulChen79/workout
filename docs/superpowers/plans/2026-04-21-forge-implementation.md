...
├── scripts/
│   └── migrate.ts               # run at container start
├── src/
│   ├── middleware.ts
│   ├── app/
│   │   ├── globals.css
...
**Files:**
- Create: `Dockerfile`, `.dockerignore`, `docker-compose.yml`, `.env.example`, `scripts/migrate.ts`
...
*Note: standalone output bundles app deps into `server.js`. We copy specific deps needed by the migrate script separately because standalone doesn't include them.*
...
- [ ] **Step 2.5: Create placeholder `scripts/migrate.ts`**
...
- Next.js standalone output omits devDependencies and non-imported packages; the Dockerfile manually copies `drizzle-orm` + `postgres` + `tsx` into the runner for the migrate script. If `npm run migrate` fails with "Cannot find module 'drizzle-orm'" at container start, that's why — double-check the copy lines in the Dockerfile.
