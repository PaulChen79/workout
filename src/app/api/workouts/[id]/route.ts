...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const [w] = await db.select().from(workouts).where(and(eq(workouts.id, workoutId), eq(workouts.userId, userId))).limit(1);
  ...
  await db.transaction(async (tx) => {
    ...
    for (const a of maxAccepts ?? []) {
      await tx.insert(exerciseMaxes).values({ userId, exerciseId: a.exerciseId, valueKg: a.newMaxKg.toString() })
        .onConflictDoUpdate({
    ...
    for (const slot of tpl.slots) {
      ...
      const [existing] = await tx.select().from(userProgressionState)
        .where(and(
          eq(userProgressionState.userId, userId),
      ...
      await tx.insert(userProgressionState).values({
        userId, exerciseId: slot.id, scheme: slot.scheme,
